...
<head>
	...
	<script type="text/javascript">
    	...
    	function updateIcon(iconName) {
    		...
    		$('#hide-p').hide();
   			$('#navIcon').fileinput({
    	        language: 'zh',
    	        maxFileCount: 1,
    	        uploadUrl: 'FrontIndex/updateNavIcon.do?iconName='+iconName,
    	        allowedFileExtensions : ['jpg', 'png','gif'],
    	    });   
   	        //这是提交完成后的回调函数    
    	...
    	function getjlmt(){
    		$.post('FrontIndex/getjlmt.do',function(data){
    			//alert(data);
    			if (''==data || '[]'==data) {
					alert("无数据");
				}else {
    	...
    	function updatejlmt(imageName){
    		...
    		inputHTML+='<input type="text" onchange="showFileInput(\'FrontIndex/updatejlmy.do\',\'updatejlmt\',{\'imageName\':\''+imageName+'\',\'imageHref\':this.value})" id="imageHref" placeholder="请输入链接地址" class="textbox textbox_295 height_35"/>';
    	...
    	function updateCourse(upOrDown,imageName) {
    		var inputHTML='<p style="color: red;margin-bottom: 5px">请输入链接地址：</p>';
    		inputHTML+='<input type="text" onchange="showFileInput(\'FrontIndex/updateCourse.do\',\'updateCourse\',{\'imageName\':\''+imageName+'\',\'imageHref\':this.value,\'upOrDown\':'+upOrDown+'})" id="imageHref" placeholder="请输入链接地址" class="textbox textbox_295 height_35"/>';
    	...
    	function loadcourse() {
    		$.post('FrontIndex/getCourse.do',function(data){
    			var up=data.up;
    			var down=data.down;
    			var courseInfo='';
    			courseInfo+='<tr>'
    	...
    	function loadNavImage() {
    		$.post('FrontIndex/getImageName.do',function(data){
    			//alert(data);
    			var imageInfo='';
    	...
    	function addNavImages() {
			...
			$('.pop_cont_input').append('<input id="nav-image" type="file" name="file" multiple/>');
   			$('#nav-image').fileinput({
    	        language: 'zh',
    	        maxFileCount: 1,
    	        uploadUrl: 'FrontIndex/addNavImage.do',
    	        allowedFileExtensions : ['jpg', 'png','gif'],
    	...
    	function deleteNavImage(imageName,thisObj) {
    		$.post('FrontIndex/deleteNavImage.do',{'imageName':imageName},function(data){
    	...
    	function updateVideo(videoImageName) {
    		...
    		$('#hide-p').hide();
   			$('#videoImageName').fileinput({
    	        language: 'zh',
    	        maxFileCount: 1,
    	        uploadUrl: 'FrontIndex/updateVideoImage.do',
    	        allowedFileExtensions : ['jpg', 'png','gif'],
